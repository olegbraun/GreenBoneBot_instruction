...
2. Вы увидите список пользователей с пагинацией (по 1 пользователю на страницу)
...
5. Для каждого пользователя доступны кнопки:
   - "Изменить код" - изменение кода пользователя
   - "Изменить уникальный номер" - изменение уникального номера участника
   - "Написать пользователю" - отправка сообщения пользователю
   - "Удалить пользователя" - удаление пользователя из базы данных
...
1. **Удаление одного пользователя:**
   - В списке пользователей найдите нужного пользователя
   - Нажмите кнопку "Удалить пользователя" рядом с пользователем
   - Пользователь будет немедленно удален из базы данных
   - Вы увидите подтверждение с информацией об удаленном пользователе
   
2. **Удаление всех пользователей:**
   - В главном меню администратора нажмите кнопку "Удалить всех пользователей"
   - Система покажет количество пользователей и запросит подтверждение
   - Нажмите "Да, удалить всех" для подтверждения или "Отмена" для отмены операции
   - После подтверждения все пользователи будут удалены из базы данных
...
- Удаление пользователей доступно только администраторам
- При удалении всех пользователей система запрашивает подтверждение для предотвращения случайного удаления
